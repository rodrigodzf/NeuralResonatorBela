...
export default class BelaData extends BelaWebSocket {
	constructor(port = 5555, address = 'gui_data', ip) {
		...
		this.events = [new CustomEvent('buffer-ready')]
	}
	...
	onData(data) {
		if ('id/type' === this.currentState) {
			...
			if (isNaN(this.newBuffer['id'])) err = true
			if (err) {
				console.log(
					'Unknown buffer type ',
					this.newBuffer['type'],
			...
			switch (type) {
				case 'c':
					...
					break
				case 'j': // unsigned int
					if (data.byteLength & 3) err = this.dataError(data)
					else {
				...
				case 'f': // float
					...
					break
				case 'd':
					if (data.byteLength & 7) err = this.dataError(data)
					else {
			...
			this.target.dispatchEvent(
				new CustomEvent('buffer-ready', { detail: this.newBuffer['id'] }),
			)
		}
	}
	...
	sendBuffer(id, type, data) {
		...
		if (this.ws.OPEN !== this.ws.readyState) return false
		this.ws.send(pkt)
		return true
	}
